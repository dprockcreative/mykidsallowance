...
class Zend_Auth_Adapter_Doctrine_Table implements Zend_Auth_Adapter_Interface
{
    ...
    protected function _authenticateValidateResultSet(array $resultIdentities)
    {
        ...
        if (count($resultIdentities) < 1) {
            ...
            $this->_authenticateResultInfo['messages'][] = 'No User by name name.';
            return $this->_authenticateCreateAuthResult();
        } elseif (count($resultIdentities) > 1) {
            $this->_authenticateResultInfo['code'] = Zend_Auth_Result::FAILURE_IDENTITY_AMBIGUOUS;
            $this->_authenticateResultInfo['messages'][] = 'Username is used by another member.';
            return $this->_authenticateCreateAuthResult();
        }
    ...
    protected function _authenticateValidateResult($resultIdentity)
    {
        if ($resultIdentity['zend_auth_credential_match'] != '1') {
            ...
            $this->_authenticateResultInfo['messages'][] = 'Password Invalid.';
            return $this->_authenticateCreateAuthResult();
        }
